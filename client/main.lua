...
function ListOwnedBoatsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedBoats', function(ownedBoats)
		if #ownedBoats == 0 then
			TriggerEvent("pNotify:SendNotification", {text =_U('garage_noboats'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
			...
			if data.current.value.stored then
				...
				TriggerEvent("pNotify:SendNotification", {text =_U('boat_is_impounded'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
...
function ListOwnedAircraftsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedAircrafts', function(ownedAircrafts)
		if #ownedAircrafts == 0 then
			TriggerEvent("pNotify:SendNotification", {text =_U('garage_noaircrafts'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			if valid then
				...
				TriggerEvent("pNotify:SendNotification", {text =_U('cannot_store_vehicle'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })

			end
		end, vehicleProps)
	else
		TriggerEvent("pNotify:SendNotification", {text =_U('no_vehicle_to_enter'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
...
function reparation(apprasial, vehicle, vehicleProps)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		...
		elseif data.current.value == 'no' then
			TriggerEvent("pNotify:SendNotification", {text =_U('visit_mechanic'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
...
function putaway(vehicle, vehicleProps)
	...
	TriggerEvent("pNotify:SendNotification", {text =_U('vehicle_in_garage'),type = "warning",queue = "duty", theme = "metroui", timeout = 2500,layout = "topRight" })
